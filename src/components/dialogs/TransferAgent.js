 ...
 * Transfer Agent
 *
 * Handles transfer dialog from provider to agent
 *
...
export default function TransferAgent(props) {
	...
	function submitTransfer(ticket_id) {
		...
		// Transfer the claim
		Claimed.transferToAgent(ticket_id, props.customerId, parseInt(agent, 10), note).then(data => {
	...
	return (
		...
		>
			<DialogTitle id="confirmation-dialog-title">Transfer to Support</DialogTitle>
